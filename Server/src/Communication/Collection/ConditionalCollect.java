...
import Communication.Messages.Consensus.CollectMessage;
import Communication.Messages.UdpMessage;
...
public class ConditionalCollect<T extends CollectMessage> {
    ...
    private final AuthenticatedPerfectLink<UdpMessage> link;
    ...
    public void receiveMessages() throws Exception {
        while (collectedMessages.size() < quorumSize) {
            MessageDeliveryTuple<UdpMessage, Integer> received = link.receiveMessage();
            if (received == null) continue; //signature couldnt be verified

            UdpMessage msg = received.getMessage();
